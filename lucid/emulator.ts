import { ActiveDelegation, Assets, Credential, EmulatorState, Hasher, Network, OutputData, OutRef, paymentCredentialOf, Provider, RelevantProtocolParameters, Utxo } from "https://deno.land/x/lucid@0.20.5/mod.ts";

export class EmulatorWithState implements Provider {
    network?: Network;
    private state: EmulatorState;
    protocolParameters: RelevantProtocolParameters;
    ...
    constructor(
      ...
      parameters: RelevantProtocolParameters = PROTOCOL_PARAMETERS_DEFAULT,
    ) {
      const time = Date.now();
      this.network = { Emulator: time };

      this.protocolParameters = parameters;
    ...
    pushUtxo(utxo: Utxo) {
      const utxos = this.state.getLedger();
      utxos.push(utxo);
      this.state = new EmulatorState(this.state.getTime(), utxos);
    }                                                                                                                                                                                                                                                                                                                                                           
